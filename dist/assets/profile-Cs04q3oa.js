import{g as s,r as c,s as n}from"./feed-DTUCvLBc.js";import{a as m,h as f}from"./authorPosts-DUoUsbdZ.js";async function d(){const o=s();if(o)try{const e=await c(o),a=document.getElementById("profile-username"),r=document.getElementById("profile-email"),t=document.getElementById("profile-bio"),l=document.getElementById("profile-avatar");a&&(a.textContent=e.name),r&&(r.textContent=e.email),t&&(t.textContent=e.bio||"No bio provided"),l&&(l.src=e.avatar?.url||"/public/images/avatar.png",l.alt=e.avatar?.alt||"User avatar")}catch(e){console.error("Error loading profile:",e)}}async function u(o,{avatar:e,bio:a}){const r=`${m}/${o}`,l=await fetch(r,{method:"PUT",headers:f(),body:JSON.stringify({avatar:{url:e,alt:"User's avatar"},bio:a})}),i=await l.json();if(!l.ok)throw new Error(i.errors?.[0]?.message||"Failed to update profile");return i.data}async function b(){const o=document.getElementById("profile-avatar").src,e=document.getElementById("profile-bio").textContent,a=`
