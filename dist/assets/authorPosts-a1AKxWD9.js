const p="8b8f0c57-e816-4800-acd0-6950ca938a84",i="https://v2.api.noroff.dev",d=`${i}/auth`,l=`${i}/social`,f=`${d}/login`,A=`${d}/register`,$=`${l}/posts`,m=`${l}/profiles`;function u(){const e=new Headers;e.append("X-Noroff-API-Key",p);const o=localStorage.getItem("accessToken");return o&&e.append("Authorization",`Bearer ${o}`),e.append("Content-Type","application/json"),e}async function h(e){const o=`${m}/${e}/posts?_author=true`,a=await fetch(o,{headers:u()}),t=await a.json();if(!a.ok)throw new Error(error.errors?.[0]?.message||"Failed to fetch user's posts");return t.data}async function g(){const e=document.getElementById("author-feed-container");if(!e)return;const a=new URLSearchParams(window.location.search).get("username");if(!a){e.innerHTML="<p>User posts not found.</p>";return}try{const t=await h(a);e.innerHTML="";const s=document.createElement("h1");s.className="mt-4 mb-2 mx-4",s.textContent=`${a}'s posts`;const r=document.createElement("p");r.className="mx-4 mb-4",r.textContent=`${t.length} post${t.length!==1?"s":""}`,e.appendChild(s),e.appendChild(r),t.forEach(n=>{const c=document.createElement("div");c.className="card mx-auto mt-4 text-white pt-1 post-card",c.innerHTML=`
          <div class="card-body">
